...
namespace engine::text
{
    ...
    // Move pos to the character after '=' in a string
    void MoveCursorToVal(uint64& pos, const std::string& line);


    /*
        Deserialize an integral value from a string
        param text: string to read
        param val: value to write into after deserialization

        return value: pointer after the value
    */
    template <math::CIntegralType TValueType>
    const char* DeserializeInteger(const char* text, TValueType& val);
    
    /* 
        Deserialize a floating point value from a string
        param text: string to read
        param val: value to write into after deserialization
    ...
    */
    ...
    // Deserialize a quaternion's w, x, y and z values from a string
    template<math::CFloatingType TValueType>
    const char* DeserializeQuaternion(const char* text, math::Quaternion<TValueType>& val);

    /*
        Deserialize a string from a custom text file
        NOTE: Do NOT call GetValuePointer before this function
    */
    const char* DeserializeString(const char* text, const char* end, std::string & val);

    /* 
        Load the contents of a text file to memory
        Note: Must open text file in binary mode for correct line feed handling, and call
        UnloadFileData when done reading text file
    */
    const char* LoadFileData(std::ifstream& file, const char*& start, const char*& end);

    // Free memory allocated by LoadFileData
    void UnloadFileData(const char* data);

    /*
        Move pointer to first character after a '=' character
        param text: current cursor (pointer to next unread character) in string
        param end: last character of entire text
        return value: pointer to first character after '=' or end
    */
    const char* GetValuePointer(const char* text, const char* end);

    /*
       Move pointer to first character after a newline character
       param text: current cursor (pointer to next unread character) in string
       param end: last character of entire text
       return value: pointer to first character after newline or end
   */
